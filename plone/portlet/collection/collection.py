...
class Renderer(base.Renderer):
    ...
    def _results(self, **kw):
        ...
        if self.data.random:
            return self._random_results(**kw)
        else:
            return self._standard_results(**kw)

    @memoize
    def _standard_results(self, **kw):
        results = []
        collection = self.collection()
        if collection is not None:
            results = collection.queryCatalog(**kw)
            if self.data.limit and self.data.limit > 0:
                results = results[:self.data.limit]
        return results

    # intentionally non-memoized
    def _random_results(self, **kw):
        results = []
        collection = self.collection()
        if collection is not None:
            """
            ...
            results = collection.queryCatalog(sort_on=None, **kw)
